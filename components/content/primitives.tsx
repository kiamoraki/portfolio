...
//
// Wraps in a `<picture>` so the browser picks AVIF → WebP → JPG/PNG
// based on its own format support (no server-side content
// negotiation needed). The matching `.avif` / `.webp` siblings are
// generated by `scripts/emit-modern-image-formats.mjs`; we only emit
// the `<source>` tags for formats whose siblings actually exist in
// `image-manifest.json`, so non-converted images (small ones, GIFs)
// fall straight through to the `<img>`.
//
// Earlier attempt at this pattern broke ~80 `.foo > img` direct-child
// CSS selectors because the `<picture>` wrapper became the new direct
// child of `.foo`. Fixed by `picture { display: contents; }` in
// globals.css — that makes the wrapper invisible to layout and to
// the descendant-combinator + nth-of-type selectors, so existing CSS
...
export function Img({ src, alt, sizes = "100vw", className }: ImgProps) {
  ...
  // Only swap in AVIF/WebP for JPG/PNG sources. GIFs intentionally
  // skip — their AVIF/WebP siblings would be first-frame-only and
  // break animations.
  ...
  if (!dims) {
    // No dimensions in manifest — fall back to a plain `<img>` with
    // no AVIF/WebP wrapping (the manifest is the source of truth
    // for whether siblings exist).
