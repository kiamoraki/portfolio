...
//
// Plain `<img>` (via next/image) — NO `<picture>` wrap. The wrap
// would break the ~85 `.foo > img` / `:nth-of-type(N)` selectors in
// `globals.css` + `project-page.css` because CSS selectors operate
// on the DOM (where `<picture>` becomes the new direct child) even
// though `display: contents` removes the picture from the layout
// tree. `display: contents` only fixes BOX-LAYOUT issues, not
// selector matching — so the layout looked right but the per-slot
// rules (image #2 in a row gets different padding, etc.) stopped
// matching and pieces fell back to default layout.
//
// The AVIF/WebP siblings on the server remain available for when the
// vhost `AllowOverride` change ships and Apache's `mod_rewrite` can
// do server-side content negotiation transparently — no markup
// change required and zero selector impact.
// ─────────────────────────────────────────────────────────────────────
